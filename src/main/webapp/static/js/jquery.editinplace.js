...
$.fn.editInPlace.defaults = {
	...
	save_button:		'<button data-inline="true" data-mini="true" data-icon="check" class="inplace_save">Save</button>  OR ', // string: image button tag to use as “Save” button
	cancel_button:		'<button data-inline="true" data-mini="true" data-icon="delete" class="inplace_cancel">Cancel</button>', // string: image button tag to use as “Cancel” button
	...
	default_text:		"<span style=\"color: red; text-decoration: underline;\" >Failed to edit, Try again.</span>", // string: text to show up if the element that has this functionality is empty
...
$.extend(InlineEditor.prototype, {
	...
	connectOpeningEvents: function() {
		...
		this.dom
			...
			.bind('click_for_edit.editInPlace', function(anEvent){ that.openEditor(anEvent); });
	},
	...
	createEditorElement: function() {
		if (-1 === $.inArray(this.settings.field_type, ['text', 'textarea', 'select', 'date', 'time']))
			throw "Unknown field_type <fnord>, supported are 'text', 'textarea' ,'select', 'date' and 'time'";
		...
		if ("select" === this.settings.field_type)
			...
			editor = $('<input type="text" ' + this.inputNameAndClass() 
				+ ' maxlength="' + this.settings.text_size  + '" />');
            ...
            editor = $('<input id="datepicker_edit" type="text" maxlength="'+ this.settings.text_size  + '"><script type="text/javascript" >'+
                ' $("#datepicker_edit").scroller({ preset:\'date\', dateOrder:\'ddmmyy\', minDate:new Date(), dateFormat:\'dd/mm/yyyy\'});'+'</script>');
        else if("time" === this.settings.field_type)
            editor = $('<input id="timepicker_edit" type="text"'+ this.settings.text_size  + '><script type="text/javascript" >'+
                ' $("#timepicker_edit").scroller({ preset:\'time\', stepMinute:5 });'+'</script>');
	...
	handleSubmitToServer: function(enteredText) {
		...
		$.ajax({
			...
			error: function(request) {
				that.dom.html(that.originalHTML).trigger('create'); // REFACT: what about a restorePreEditingContent()
                ...
                var new_text = response || that.settings.default_text;
